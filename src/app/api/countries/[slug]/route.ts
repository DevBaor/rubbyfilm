...
export async function GET(
  ...
  const rateLimit = checkRateLimit(`country_${ip}`, 120, 60);
  ...
  try {
    ...
    if (!isValidSlug(cleanSlug)) {
      return apiError("INVALID_COUNTRY", "A valid country slug is required", 400);
    }
    ...
    const result = await movieService.getMoviesByCountry(cleanSlug, { page, limit });
